...
- `debug.sf.disable_client_composition_cache=0` (`vendor.prop:255`) — client composition cache was eating UI thread time on this mt6781. Flipping to 0 reduces SystemUI jank ~10% under saver (8.22% → 7.44%) and vsync misses 16→15. A/B measured via scratchpad/bench.sh (10 runs each, warm-only).
- `debug.sf.frame_rate_multiple_threshold=60` (`vendor.prop:245`) — 60Hz panel, threshold=90 caused phantom frame-rate-mode switches. Drop to 60 reduces jank 8.22% → 7.06% (isolated), 6.63% combined with compcache0. A/B measured same way.
